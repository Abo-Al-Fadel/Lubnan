...
function LanguageSwitch({ tone = 'hero' }: { tone?: 'hero' | 'page' }) {
  const { locale, setLocale, tr } = useSite();
...
function ThemeToggle({ tone = 'hero' }: { tone?: 'hero' | 'page' }) {
  ...
  return (
    <button
      ...
      className={`flex h-10 w-10 items-center justify-center rounded-full border ${ring} ${ink} transition-colors duration-200 ease-out hover:border-[color:var(--nav-accent)] hover:bg-[color:var(--nav-accent)] hover:text-[color:var(--nav-accent-ink)]`}
    >
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between gap-4 px-5 py-5 md:px-10 md:py-7">
        <Link href="/" className={`vt-wordmark ${ink}`} aria-label={tr('nav.home')}>
          <Wordmark size="md" />
        </Link>
        ...
        <div className="flex items-center gap-2 xl:hidden">
          <button
            ...
            className={`flex h-10 w-10 items-center justify-center rounded-full border ${ring} ${ink}`}
          >
            <SearchIcon />
          </button>
          <button
            ...
            {NAV.map(([key, href]) => (
              <li key={key}>
                <Link
                  ...
                  className={`font-display text-xl uppercase tracking-wide ${ink}`}
                >
                  {tr(key)}
                </Link>
              </li>
